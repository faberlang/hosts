...
//! literal reference is the arena handle). The U6-A cursor-stream fixtures
//! (`cede/cede.fab`, `cursor/cursor.fab` — regenerated by `radix emit -t
//! wasm-text` and checked in at `fixtures/cede.wat` + `fixtures/cursor.wat`)
//! prove the U6-B materialization binding end-to-end against their checked-in
//! `.expected` oracles. Reject cases assert the typed
...
/// `cede/cede.fab` — regenerated verbatim by `radix emit -t wasm-text
/// radix/corpus/cede/cede.fab` (U6-A bde336445 landed the cursor-stream
/// emit) and checked in at `fixtures/cede.wat` — runs through
/// `WasmRtV1Host` and its captured stdout matches the checked-in sibling
/// `fixtures/cede.expected` oracle (`[1, 2]`). This is the U6-A generator
...
fn cede_fixture_compiler_artifact_matches_expected() {
    let bytes = wat_bytes(include_str!("fixtures/cede.wat"));
    let expected = include_str!("fixtures/cede.expected");
    let outcome = host().run(&bytes, &RunConfig::default());
    assert_eq!(
        outcome,
        RunOutcome::Success {
...
/// Pair verification (U6-A + U6-B): the REAL compiler artifact for
/// `cursor/cursor.fab` — regenerated verbatim by `radix emit -t wasm-text
/// radix/corpus/cursor/cursor.fab` (U6-A bde336445 landed the cursor-stream
/// emit) and checked in at `fixtures/cursor.wat` — runs through
/// `WasmRtV1Host` and its captured stdout matches the checked-in sibling
/// `fixtures/cursor.expected` oracle (`[1, 2]`).
#[test]
fn cursor_fixture_compiler_artifact_matches_expected() {
    let bytes = wat_bytes(include_str!("fixtures/cursor.wat"));
    let expected = include_str!("fixtures/cursor.expected");
    let outcome = host().run(&bytes, &RunConfig::default());
    assert_eq!(
        outcome,
        RunOutcome::Success {
