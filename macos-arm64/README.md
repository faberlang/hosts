...
- provide Faber host capability imports for flat `norma:*` modules,
...
- WASI world selection and Faber host import ABI are not locked yet.
- Wasmtime is used for the current core Wasm and component route proofs only.
- No Faber host import ABI is locked yet.
...
- Should host imports use direct Faber WIT packages or map one-to-one onto existing WASI
