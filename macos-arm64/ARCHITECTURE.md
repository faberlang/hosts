...
**Coordinating Docs**: [`../../docs/wasm-execution-plan.md`](../../docs/wasm-execution-plan.md), [`../../docs/frame-native-concurrency-vision.md`](../../docs/frame-native-concurrency-vision.md)
...
The `.fab` files under sibling `norma/src` can remain useful as interface definitions, metadata, and canonical Faber-facing contracts. They should not imply that all implementation must be bundled into each compiled artifact.
...
**Frame `data` carrier.** The macOS host types `Frame.data` as `faber::Valor`.
Keyed syscall arguments are named fields inside a `valor` tabula. JSON parsing
and emission happen only at CLI/debug transport boundaries (`valor_wire`), not as
the in-memory payload type.
...
- classify sibling `norma/src` files into compiler core, host capability, or temporary Rust backend support,
- move provider surfaces toward host capability contracts,
